...
namespace EvolveGames
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        public AudioClip[] step_clips;
        public float activeStepDellay;
        ...
        private bool ReadyForStepSFX()
        {
            return horizontal != 0 || vertical != 0 && characterController.isGrounded && !isCrough;
        }
        ...
        void Update()
        {
            ...
            if (ReadyForStepSFX())
            {
                activeStepDellay = isRunning ? fastSpeed : slowSpeed;
                stepTimer += Time.deltaTime;
                if (stepTimer > activeStepDellay)
                {
                    int rand = Random.Range(0, step_clips.Length);
                    sfx.PlayOneShot(step_clips[rand]);
                    stepTimer = 0;
                }
            }
            else if (stepTimer != 0)
